    ...
    <div className="page">
      <section className="min-h-[55vh] flex flex-col justify-center stagger">
        ...
        <p className="mt-6 text-lg text-dim leading-relaxed max-w-xl">
          PrivateConnect discovers which of your contacts are already on the
          platform — without ever uploading your address book in plaintext.
          Powered by Arcium&apos;s confidential Multi-Party Computation.
        </p>
        ...
        <div className="mt-10 flex flex-wrap gap-3">
          ...
          <Link href={ROUTES.DISCOVER} className="btn-ghost">
            Try the demo
          </Link>
        </div>
      </section>
      ...
      <section className="mt-20 grid grid-cols-1 sm:grid-cols-3 gap-4 stagger">
        {STATS.map((s, i) => (
          <div
            key={i}
            className="card text-center space-y-1 hover:border-arc/20 transition-colors"
          >
      ...
      <section className="mt-20 rounded-2xl border border-arc/20 bg-arc/5 p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div className="space-y-1">
          <h2 className="text-xl font-bold text-bright">
            Ready to discover privately?
          </h2>
          <p className="text-sm text-dim">
            Import up to 10 contacts. Results in seconds. Your list stays
            yours.
          </p>
        </div>
        <Link href={ROUTES.ONBOARD} className="btn-primary shrink-0">
          Get started →
        </Link>
      </section>
    </div>
